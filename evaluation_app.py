# Thesis Evaluation Tool — KHCC (Blinded, No Gemini, Demo-ready)
...
# Page config
# -------------------
st.set_page_config(page_title="Thesis Evaluation Tool — KHCC (Blinded)", layout="wide")
...
# -------------------
# Sanitizer: remove model/brand mentions from any note text
# -------------------
_SANITIZE_TERMS = [
...
def sanitize_note(text: str) -> str:
    if not isinstance(text, str):
        return ""
    # remove common "Assistant/Model says:" prefixes
    text = re.sub(r"(?i)\b(model|assistant)\s+says?:", "", text)
    # redact vendor/model names
    text = _SANITIZE_RE.sub("[redacted]", text)
    return text.strip()

# -------------------
# Demo cases (neutral text, no model names)
# -------------------
...
def _init_state():
    ...
    ss.setdefault("wizard_mode", True)              # ask rubric Qs one-by-one
    ss.setdefault("blind_map", {})                  # per-case mapping: {'KHCC-001': {'A':'chatgpt_note', ...}}
...
def load_cases(upload) -> pd.DataFrame:
    ...
    for c in EXPECTED:
        if c not in df.columns:
            df[c] = ""
    for c in ["chatgpt_note", "claude_note", "cadss_note", "human_note"]:
        df[c] = df[c].astype(str).apply(sanitize_note)
...
def get_blind_mapping_for_case(case_row: pd.Series) -> dict:
    """Return mapping like {'A': 'chatgpt_note', 'B': 'claude_note', 'C': 'cadss_note'} for available notes, shuffled."""
    available = []
    for col in ["chatgpt_note", "claude_note", "cadss_note"]:
        if (str(case_row.get(col, "")) or "").strip():
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Fully blinded: evaluators only see **Note A/B/C**. Model identities are never displayed or exported.")

# -------------------
# Main layout
# -------------------
st.title("Thesis Evaluation Tool — KHCC (Blinded)")
st.caption("Evaluating AI-generated pharmacist notes for breast cancer chemotherapy. Gemini removed; strict blinding enforced.")
...
with tab_home:
    ...
    with cols[0]:
        ...
        st.markdown("**2) Open a Note**\nNotes appear as **Note A/B/C** (randomized per case).")
    with cols[1]:
...
with tab_evaluate:
    ...
    if not st.session_state.evaluator.strip():
        ...
        with left:
            ...
            st.write(sanitize_note(row.get("patient_summary", "")) or "_(empty)_")

        with right:
            st.markdown("### Human Pharmacist Note (reference)")
            st.write(sanitize_note(row.get("human_note", "")) or "_(empty)_")
        ...
        if not note_labels:
            ...
            note_text = sanitize_note(row.get(model_col, "")) or "_(empty)_"
            ...
            if st.session_state.wizard_mode:
                ...
                for i, (label, (w, help_txt)) in enumerate(rubric_items, start=1):
                    ...
                    subscores[label] = st.slider(
                        f"{i}. {label}", *RANGE, value=0,
                        help=f"Weight {int(w*100)}%. {help_txt}"
                    )
                    section_comments[label] = st.text_area(
                        f"Comment — {label}", height=70, placeholder="Optional", key=f"c_{label}"
                    )
                    st.divider()
            else:
                cols = st.columns(2)
            ...
            if st.button("✅ Save evaluation", use_container_width=True):
                entry = {
                    ...
                    "note_label": lab,   # A/B/C only — model identity never stored
                    "base_score": base,
                    "final_score": final,
...
with tab_review:
    ...
    if not st.session_state.evaluations:
        ...
        with c3:
            sort_by = st.selectbox("Sort by", ["timestamp", "final_score", "base_score", "case_id", "note_label"])
...
with tab_export:
    ...
    if not st.session_state.evaluations:
        ...
        export_cols = [
            ...
            "case_id", "phase", "note_label",  # A/B/C only
        ...
        st.caption("Export remains blinded (Note A/B/C). Model identities are not stored anywhere in the file.")
